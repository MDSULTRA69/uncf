...
import {
  ...
  createMove, createClan, createRule, createGem, createVillage, updateVillage
} from '../utils/api';
...
export default function AdminPanel() {
  ...
  const tabs = [
    ...
    { id: 'moves', label: 'Add Move', icon: Swords },
    { id: 'clans', label: 'Add Clan', icon: Shield },
    { id: 'rules', label: 'Add Rule', icon: BookOpen },
    { id: 'gems', label: 'Add Gem', icon: Gem },
    ...(user?.role === 'npc' ? [{ id: 'villages', label: 'Villages', icon: Map }] : []),
  ];

  const FormCard = ({ title, children, onSubmit, buttonLabel = 'Create' }) => (
    <div className="card" style={{ maxWidth: '600px' }}>
      <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.85rem', color: '#e2b96f', marginBottom: '1.5rem', letterSpacing: '0.1em' }}>{title}</div>
      {children}
      <button className="btn btn-gold" onClick={onSubmit} disabled={loading} style={{ marginTop: '0.5rem' }}>
        <Save size={15} /> {loading ? 'Saving...' : buttonLabel}
      </button>
    </div>
  );
  ...
  return (
    <div className="fade-in">
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontFamily: 'Cinzel Decorative, serif', fontSize: '1.4rem', color: '#e74c3c' }}>Admin Panel</h1>
        <p style={{ color: '#5a5a7a', fontSize: '0.8rem', marginTop: '0.25rem' }}>Role: <span style={{ color: '#9b59b6', textTransform: 'capitalize' }}>{user?.role}</span></p>
      </div>

      {/* Tab nav */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginBottom: '1.5rem' }}>
        {tabs.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => setTab(id)} className={`btn btn-sm ${tab === id ? 'btn-red' : 'btn-ghost'}`}>
            <Icon size={13} />{label}
          </button>
        ))}
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
          {[
            ...
            <div key={label} className="card" style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '0.72rem', color: '#5a5a7a', fontFamily: 'Cinzel, serif', letterSpacing: '0.1em', marginTop: '0.25rem' }}>{label}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {tab === 'players' && (
        <div className="card">
          <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.85rem', color: '#e2b96f', marginBottom: '1rem' }}>ALL REGISTERED PLAYERS ({players.length})</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: '600px', overflowY: 'auto' }}>
            {players.map(p => (
              <div key={p._id} style={{ background: '#0a0a12', border: '1px solid #1e1e32', borderRadius: '6px', padding: '0.875rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                    <div style={{ fontWeight: 600, color: '#e8e0d0' }}>{p.characterName}</div>
                    <span style={{ fontSize: '0.68rem', color: '#5a5a7a' }}>@{p.username}</span>
                    <span className={`badge ${p.role === 'npc' ? 'badge-red' : p.role === 'kage' ? 'badge-gold' : 'badge-cyan'}`} style={{ fontSize: '0.6rem' }}>{p.role}</span>
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    <span className="badge badge-gold" style={{ fontSize: '0.65rem' }}>{p.rank}</span>
                ...
                <div style={{ fontSize: '0.72rem', color: '#5a5a7a', marginTop: '0.3rem' }}>
                  {p.clan} · {p.village} · W:{p.stats?.wins || 0} L:{p.stats?.losses || 0} · {p.stats?.xp?.toLocaleString() || 0} XP
                </div>
                {editingPlayer === p._id && (
                  <div style={{ marginTop: '0.875rem', paddingTop: '0.875rem', borderTop: '1px solid #1e1e32' }}>
